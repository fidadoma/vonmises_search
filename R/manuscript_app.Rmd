...
df_exp1b <- 
  ...
  rename(tagetspd = type) %>% mutate(id = factor(subject)) %>% 
...
p
ggsave(file.path(plot_pth, "Figure3.png"), p, width = 9, height = 6)
```
...
accu_summaryx %>% filter(contrast_abs >= 2, target_faster== F) %>% summarize(m = mean(accu), sd = sd(accu)) %>% knitr::kable(digits = 2)
...
ggsave(file.path(plot_pth, "Fig5b_sp05.eps"), p_sp05, width = 6, height = 6)
ggsave(file.path(plot_pth, "Fig5b_sp1.eps"), p_sp1, width = 6, height = 6)
ggsave(file.path(plot_pth, "Fig5b_sp2.eps"), p_sp2, width = 6, height = 6)
ggsave(file.path(plot_pth, "Fig5b_sp4.eps"), p_sp4, width = 6, height = 6)
ggsave(file.path(plot_pth, "Fig5b_sp8.eps"), p_sp8, width = 6, height = 6)
...
p <- accu_summaryx %>% 
  ...
  xlab("contrast") + ylab("Accuracy")
p
ggsave(file.path(plot_pth, "Figure6.png"), p, width = 9, height = 6)
...
accu_summaryx %>%
  ...
  do(ez::ezANOVA(., dv = accu, wid = id, within = target_faster, detailed = T)$ANOVA) %>% 
  filter(Effect != "(Intercept)") %>% 
  mutate(p = round(p, 3), ges = round(ges,2), F = round(F,1)) %>% 
  transmute(`Contrast level` = contrast_abs, `F(1,15)`=F,p, ges) %>% 
  knitr::kable()
```
