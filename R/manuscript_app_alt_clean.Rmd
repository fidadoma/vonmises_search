...
df_exp1b <- 
  ...
  mutate(contrast_abs = abs(log2(targetk / restk)), 
         ...
         ratio = (besselI(targetk,0) * exp(targetk)) / (besselI(restk,0) * exp(restk)),
         ratio_abs = abs(log((besselI(targetk,0) * exp(targetk))) / (besselI(restk,0) * exp(restk))))
participants_exp1b <- 
...
accu_summaryx %>% 
  ...
  theme(aspect.ratio = 1) + 
  xlab("kl distance") + ylab("Accuracy") +scale_x_log10()
```
...
accu_summaryx %>% 
  ...
  theme(aspect.ratio = 1) + 
  xlab("kl distance") + ylab("Accuracy") +scale_x_continuous(trans ="log")
```
